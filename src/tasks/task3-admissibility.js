...
export function initT3(){
  ...
  let nmc=2, npairs=600, nseeds=3, targetPeak=0.6, EPS=0.2, BATCH=16, policyMode="off_on", DEPTH=4;
  let lastRun=null;
  let lastSweep=null;   // {out, nms} from the most recent sweep, for the per-n_mc viewer
  ...
  function cStats(rk){
    ...
    for(let l=0;l<DEPTH;l++)for(let s=0;s<SN;s++){
      ...
      Sds.push(policyMode==="off" ? Math.abs(Euni-E) : Math.sqrt(Math.max(E2-E*E,0)));
    }
  ...
  function trainOne(regK, nmcUse, data){
    ...
    const snapshot=()=>Array.from({length:DEPTH},(_,l)=>Array.from({length:SN},(_,s)=>polAt(l,s)));
    const gap=()=>{
      let g=0;
      for(let l=0;l<DEPTH;l++)for(let s=0;s<SN;s++){
    ...
    /* trajectory step: {l,s,a,sn} (off / off_on) or {l,s,acts,i,sn} (on). chosen action: */
    const chosenA=st=>st.acts?st.acts[st.i]:st.a;
    /* C_Ω(π(·|s')) over a given action set — the only inner formula (KL→const by Φ_KL≡1). */
    const innerValue=(pol,acts,ref)=>{
      if(R.constC||acts.length===0)return{C:0,as:acts};
      let e=0;
      if(regK==="euc"){for(const aj of acts)e+=pol[aj]-ref[aj];return{C:e/acts.length-R.omega(pol,ref),as:acts};}
      for(const aj of acts)e+=R.Phi(Math.max(pol[aj],1e-12)/ref[aj]);
      return{C:e/acts.length,as:acts};
    };
    /* actions feeding C at next state s', per policyMode:
       off = logged a' · on = stored {a_j} sampled at s' · off_on = n_mc fresh draws from current π(·|s') */
    const innerActions=(trans,i,polNext)=>{
      if(policyMode==="off"){const aN=(i+1<trans.length)?chosenA(trans[i+1]):null;return aN==null?[]:[aN];}
      if(policyMode==="on")return trans[i+1].acts;
      const as=[];for(let j=0;j<nmcUse;j++)as.push(sampleCat(polNext));return as;
    };
    const scoreTraj=(trans,pols)=>{
      let sc=0;const inner=[];
      for(let i=0;i<trans.length;i++){
        const st=trans[i],l=st.l,s=st.s,sn=st.sn,a=chosenA(st);
        sc+=aReg*R.gradDPO(pols[l][s],refOf(l,s))[a];
        if(sn>=0){
          const acts=innerActions(trans,i,pols[l+1][sn]);
          const di=innerValue(pols[l+1][sn], acts, refOf(l+1,sn));
          sc+=aReg*ISIGN*di.C;
    ...
    /* on-policy (cat2): a lagged sampler π_θ rolls out trajectories; store n_mc actions + chosen index,
       reused both to continue the trajectory and to estimate C. n_mc=1 ⇒ ordinary trajectory. */
    const rolloutOn=sampler=>{
      let s=Math.floor(Math.random()*SN);const trans=[];let Rret=0;
      for(let l=0;l<DEPTH;l++){
        const p=sampler[l][s];const acts=[];for(let j=0;j<nmcUse;j++)acts.push(sampleCat(p));
        const i=Math.floor(Math.random()*nmcUse),a=acts[i];
        Rret+=Math.pow(GAMMA,l)*rewards[l][s][a];
        const sn=l<DEPTH-1?sampleCat(transP(a,EPS)):-1;
        trans.push({l,s,acts,i,sn});s=sn;
      }
      return{trans,R:Rret};
    };
    const makeDataOn=(sampler,nPairs)=>{
      const d=[];
      for(let k=0;k<nPairs;k++){const t1=rolloutOn(sampler),t2=rolloutOn(sampler);
        const pw=1/(1+Math.exp(-(t1.R-t2.R)));
        d.push(Math.random()<pw?{w:t1.trans,l:t2.trans}:{w:t2.trans,l:t1.trans});}
      return d;
    };
    /* "on" (cat2): generate own data from a lagged π_θ snapshot, regenerate every RESAMPLE_EVERY steps.
       off / off_on use the passed-in (off-policy) `data`. */
    const RESAMPLE_EVERY=25, ON_PAIRS=Math.min(npairs,600);
    let trainData = policyMode==="on" ? makeDataOn(snapshot(),ON_PAIRS) : data;
    const curve=[];let t=0;
    for(let step=0;step<STEPS;step++){
      if(policyMode==="on"&&step>0&&step%RESAMPLE_EVERY===0)trainData=makeDataOn(snapshot(),ON_PAIRS);
      ...
      for(let b=0;b<BATCH;b++){
        const pair=trainData[Math.floor(Math.random()*trainData.length)];
        ...
        const accOuter=(trans,sign)=>{
          for(const st of trans){
            const l=st.l,s=st.s,a=chosenA(st);
            const pol=pols[l][s];
  ...
  /* render one n_mc case (training curves + policy panels) and tag the titles with the n_mc */
  function showNmc(out,nm){
    drawCurves(out[nm]);
    drawPolicies(out[nm],nm);
    const ct=$("t3-curve-title");
    if(ct)ct.textContent=`Training curves — policy gap = mean TV(π_θ, π*_Ω) over states · n_mc=${nm} (lower is better, seed mean)`;
  }
  /* segmented n_mc selector shown after a sweep: click a case to view its curves & policy panels */
  function buildNmcSel(out,nms,cur){
    const wrap=$("t3-nmcsel-wrap"),seg=$("t3-nmcsel");
    if(!wrap||!seg)return;
    seg.innerHTML="";
    nms.forEach(nm=>{
      const b=document.createElement("button");b.textContent="n_mc = "+nm;
      if(nm===cur)b.classList.add("on");
      b.addEventListener("click",()=>{seg.querySelectorAll("button").forEach(x=>x.classList.remove("on"));b.classList.add("on");showNmc(out,nm);});
      seg.appendChild(b);
    });
  ...
  $("t3-offp").querySelectorAll("button").forEach(b=>b.addEventListener("click",()=>{
    ...
    policyMode=b.dataset.v;                          // "off" | "off_on" | "on"
    const nmcIgnored=(policyMode==="off");
    $("t3-nmc").disabled=nmcIgnored;
    $("t3-nmc").style.opacity=nmcIgnored?0.4:1;
    const ct=$("t3-cstats-title");
    if(ct)ct.textContent=(policyMode==="off")
    ...
    $("t3-status").textContent=
      policyMode==="off" ? "Off-policy (cat 1) — inner term from the single logged a' (n_mc ignored). Sharpest: only the admissible Ω is unbiased from data alone."
      : policyMode==="on" ? "On-policy (cat 2) — a lagged sampler rolls out the trajectories; the stored n_mc actions both estimate C and continue the trajectory (regenerated every 25 steps)."
      : "Off-on-policy (cat 3, Dyna) — off-policy data states, but a' is freshly resampled (n_mc) from the current π(·|s'). The n_mc slider matters.";
    drawCStats();
  }));
  ...
  function summarize(out){
    const o={meta:{targetPeak,EPS,BATCH,DEPTH,policyMode,npairs,nseeds,STEPS,GAMMA,LR},
  ...
  $("t3-run").addEventListener("click",()=>{
    ...
    runAll([nmc],out=>{
      ...
      lastSweep=null;$("t3-nmcsel-wrap").style.display="none";   // single run: no per-case selector
  ...
  $("t3-sweep").addEventListener("click",()=>{
    ...
    runAll([1,4,16],out=>{
      lastRun=summarize(out);
      lastSweep={out,nms:[1,4,16]};
      drawBars(out,"n_mc sweep — final policy gap (± seed std) · pick a case below for its curves & policy panels");
      buildNmcSel(out,[1,4,16],1);
      showNmc(out,1);
      $("t3-status").textContent="Sweep done — use the “View n_mc case” buttons above the curves to inspect each case’s training curve & policy recovery. KL stays flat; non-KL improve as n_mc grows.";
    });
  });
  const stamp=()=>`peak${targetPeak.toFixed(2)}_eps${EPS.toFixed(2)}_b${BATCH}_L${DEPTH}_${policyMode}`;
